...
| **Epic [#39](https://github.com/chienchuanw/only-cue/issues/39) — templates** | **4 of 5 leaves.** Save the project's `CuePointType` set as a `.cuelist-template` under `~/Documents/OnlyCue/Templates/`; File → Load Template… merges a template into the open project (append + fresh UUIDs so existing cues' `typeID` references never break — ADR-015). New-document-from-template UX is the remaining leaf. |
...
- **Phase 2 — Pro handoff** — nine epics filed; #32, #34 and #35 complete; #36, #38 and #39 in flight. Open epics not yet started: [#33](https://github.com/chienchuanw/only-cue/issues/33) LTC, [#37](https://github.com/chienchuanw/only-cue/issues/37) timeline breakdown view, [#40](https://github.com/chienchuanw/only-cue/issues/40) custom shortcuts editor.
