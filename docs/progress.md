...
## 2026-09-17 — The LTC *decoder* contract, and two mutants that had to be hunted (#862)

**Shipped to `dev` (PR #863, rebase-merged, closes #862):**

- `chore(windows)`: slices 1 and 2 pinned what OnlyCue **emits**; this one pins what it **believes when it reads**. `golden/ltc-decode-v1.json` — 170 cases, 34 signals × 5 pipeline ops (`transitions` / `halfBit` / `framesPerSecond` / `bits` / `decode`) — with an `LtcDecoder` mirror in `OnlyCue.Core`. Swift 1872 tests (was 1863), C# 1055 (was 871 — 170 vectors + 11 hand pins + 3 equivalence guards). The LTC path is now pinned in both directions on both platforms.
- Signals are **recipes, not sample dumps**: both sides build an identical `[Float]` from the input fields in a fixed mutation order, and `expect.sampleCount` fingerprints the recipe so a divergence in *signal construction* cannot masquerade as a divergence in the decoder. Corruption is structural only — truncation, flipped bits, silence, DC — because reproducing added noise needs a seeded PRNG whose output becomes part of the contract, which pins the PRNG rather than the decoder.
- Five ops per signal rather than one, because with only `decode` to assert on, every hazard in the pipeline fails the same way and a mutation cannot tell a broken comparator from a broken framer.
- **Mutants 8 and 9 survived the first full run, and the spec's rule 6 is the reason this slice is worth reading.** The spec predicted `parity-flipped-middle-frame` would kill both (moving the 80-bit advance inside the validity check; never skipping at all). It kills neither — with sync words only ever at frame boundaries, a decoder that crawls bit-by-bit through the broken frame simply **re-locks on the next real sync word** and returns a byte-identical list. The three implementations are indistinguishable on that signal.
  - They were then proven **non**-equivalent by constructing a distinguishing input at the bit level, and the contract grew rather than the finding being written off: `spurious-sync-after-a-broken-frame` breaks frame 2's parity and plants a **sync word inside frame 3's payload**, deliberately off the frame grid. The correct decoder skips 80 bits past the wreck and never sees the plant; a crawling one locks onto it and reports `00:00:45:00` — a frame that was never transmitted. Both mutants now die on exactly two tests.
  - Finding the plant in the test needed one more correction: the demodulated stream does **not** begin on a frame boundary (the comparator's first latch is not recorded, so roughly one bit is lost, not a whole frame). The assertion reads the grid off the stream — modal sync-match residue mod 80 — instead of computing it, which is also a stronger statement of the property.
  - Both comments that carried the falsified claim (`LtcDecoder.ExtractFrames`, `LTCDecodeGolden.parityFlipBit`) were corrected **in place**, naming what the case does *not* prove.
- **Mutants 2, 5 and 6 survive and are equivalent only over this matrix's signals** — the mutated boundary always falls in a gap the signals never reach. That is a property of the matrix, not of the code, so it is asserted per signal in `LtcDecoderMutationEquivalenceTests` rather than argued once in a merged PR body: measured margins are 9.44e4 widening-gaps (threshold width), 3.5 samples (the `1.5×` cluster boundary), and 0.363 from the nearest rounding tie. Add a signal that reaches one of those boundaries and the guard goes red, which is the notice that the survivor has become a real gap.
  - On the spec's extra instruction for mutant 6 — a `.5` tie in `bitRate / 80` is **not reachable through the recipe format at all**, because a recipe declares one sample rate used both to synthesise and to decode. A tie needs a signal generated at one rate and decoded as another, i.e. real off-rate playback, which v1 does not claim to support. The rounding mode stays contract; the distance to the nearest tie is measured rather than assumed to be large.
- Two more predictions were wrong in detail: mutant 1 (`rms` in `float`) dies on the silence-floor ladder, not the long 30-frame case; mutant 4 (windowed RMS) dies on the DC ladder, not `silence-lead-in`. Still dead, so no action — recorded so the next slice doesn't re-derive the surprise.
- Deviations from the spec's sketch: reference `frameCount` is **6, not 4** (the decoder structurally loses the leading and trailing frame, so a 4-frame run decodes 2 and leaves `parity-flipped-middle-frame` with no "frames after it" to check), and `Input.flipBitInFrame` / `flipBitIndices` became `flips: [FrameFlip]` because the spurious-sync case mutates two frames.
- First use of **`InternalsVisibleTo`** in the repo, called out rather than slipped in: the Swift helpers are `private`, and pinning an intermediate is not a reason to widen the shipping surface. `ThresholdFraction` and `Rms` are `internal` for the same reason plus one more — the equivalence proof needs the exact threshold and reference amplitude the comparator saw, not a re-derivation.
- Verification counted, not inferred from the green tick: the Windows CI log (run `35125657778`, Release) shows `LtcDecodeGoldenVectorTests.CSharpCore_ReproducesGoldenCase` executed **170** times — 34 per op across all five — plus the 11 hand pins and the 3 equivalence guards, totalling `1055 / 1055`.
- Still **M3 — hardware**, and unlike slices 1 and 2 the issue title says M3 this time. M1e (`OnlyCue.App.ViewModels` + the WinUI shell) remains blocked on a real Windows machine, which is why the pure-logic slices keep getting pulled forward.

---

## 2026-09-16 — The LTC/MTC *schedule* contract, and two spec predictions that were wrong (#859)

**Shipped to `dev` (PR #860, rebase-merged as `8af1002` head, closes #859):**

- `chore(windows)`: slice 1 pinned the wire format; this slice pins how those bytes and samples are laid out **in time**. `golden/ltc-schedule-v1.json` (129 cases) and `golden/mtc-schedule-v1.json` (83 cases), with `LtcFrameStream` / `LtcSchedule` / `MtcSchedule` mirrors in `OnlyCue.Core`. Swift 1863 tests (was 1852), C# 871 (was 644 — 212 vectors + 15 hand-computed pins).
- Three hazards drove the matrix, all of them arithmetic that *looks* portable. `samplesPerBuffer` rounds **per frame then multiplies** (44.1 kHz @ 24 fps ⇒ `4 × round(1837.5) = 7352`, not 7350). `framesPerBuffer(forTargetSeconds:)` and `timestampForQuarterFrame` both need half-away-from-zero (0.1 s @ 25 fps ⇒ 3, 0.5 s ⇒ 13; banker's says 2 and 12). And `MTCSchedule` **quantises before it ceils** — dropping that loses 74 of the first 200 messages at 24 fps and 66 at 30 fps, and **0** at 25 fps where the quarter-frame period is exactly 10 000 000 ns, which is why the contract carries tiled `batchChain` cases at all three rates with 25 fps as the control.
- **Two of the spec's mutation predictions were falsified, and both are recorded rather than quietly dropped.** The mutation list is a set of hypotheses; treating a survivor as "the test is too weak" would have been the wrong reflex twice over.
  - **Mutant 6 — resetting `LTCFrameStream`'s polarity per frame — is an *equivalent* mutant.** The spec said it "must fail the `joins` windows … if it survives, the window is too narrow". It survived and the window is fine: the bit-polarity-correction bit forces an even number of ones, hence an even number of mid-bit flips, so **every LTC frame ends at the level it started at** and threading vs. resetting emit byte-identical PCM. The *real* fault is an **inverted** join, which drops a transition; that is mutant 6b, killed by 17 `stream` / `buffer` / `bufferSeam` cases, and it is what actually justifies the ±16-sample window. `test_joinWindow_catchesAnInvertedJoin_butResettingOneIsEquivalent` now asserts **both** halves so the claim cannot rot back.
  - **Mutant 2's inner quantise rounding is unobservable.** The spec claimed a plain `Math.Round` in *either* position reintroduces the message loss. Only the outer `Ceiling` does. Proven rather than argued: enumerating every clock × rate pair in the contract over the first 200 000 quarter-frames — each emitted timestamp plus one tick either side, 7.2 M values — finds no exact `.5` tie. `MidpointRounding.AwayFromZero` stays because mirroring the source of truth costs nothing, and the doc comment now says no vector can pin it.
- The falsified claim had been written into **three** comments (`LTCSchedule.swift`, `LTCScheduleGolden.swift`, `LtcFrameStream.cs`). The Swift one said the buffer-seam reset is "harmless, since an LTC reader keys on transitions, not absolute polarity" — reasoning that would equally excuse a genuinely inverted seam. All three now state the parity argument, which is narrower and actually true.
- Deviation from the spec's sketch: `op: "buffer"` dropped its `joins` (redundant with `op: "stream"`, same joins at the same windows); `op: "bufferSeam"` carries the PCM either side of a buffer boundary instead — the thing `buffer` alone could not see.
- Verification was counted, not inferred from the green tick: the Windows CI log (run `35106827453`, Release config) shows `LtcScheduleGoldenVectorTests.CSharpCore_ReproducesGoldenCase` executed **129** times and the MTC twin **83**, totalling `871 / 871`.
- Still **M3 — hardware**, not M2, despite the issue title saying "M2 slice 2" (same mislabel as slice 1). M1e (`OnlyCue.App.ViewModels` + the WinUI shell) remains blocked on a real Windows machine, which is why the pure-logic slices keep getting pulled forward.
...
- Quarter-frames go out as a batched look-ahead window with **future** timestamps, so CoreMIDI's scheduler does the timing and a main-actor stall can only delay the next refill, never delivery of what is already queued. `MIDISend`/`MIDIPacketList` take raw MIDI 1.0 bytes but were deprecated in macOS 11 and Release treats warnings as errors, so the send path speaks UMP — that bit-packing is pure and tested rather than buried in the untestable edge.
- Two deliberate departures from the LTC path: **any** seek re-cues (not only one over 1.0 s — one Full Frame vs. a buffer re-prime), and the playhead is followed **while paused** so a console parked on a cue shows the right time before go.
...
- **Brief from the user:** when the project framerate is configured, all time-related displays (including the Inspector clock) should render in that framerate format instead of using milliseconds as the smallest unit.
- **Decisions captured in spec `docs/superpowers/specs/2026-05-15-framerate-time-display-design.md`:** all time displays switch; origin is project-relative (`00:00:00:00` = playback start, per-media `startTimecodeFrames` reserved for the existing LTC-aligned readout); no `.mmm` opt-out (YAGNI — `ProjectTimecodeSettings.framerate` always exists, default 30 fps); countdowns also frame-formatted (`SS:FF` / `M:SS:FF` / `H:MM:SS:FF`).
- **Implementation:** new `TimeFormat.smpte(_:rate:)` and `TimeFormat.smpteCountdown(_:rate:)` wrap the existing `Timecode(totalSeconds:rate:).displayString` (drop-frame `;` separator inherited). Legacy `TimeFormat.hms` and `TimeFormat.compactCountdown` removed. New `EnvironmentValues.projectFramerate: SMPTEFramerate` (default `.fps30`) seeded once at `DocumentView` body root from `document.model.timecodeSettings.framerate`; leaf views read it via `@Environment(\.projectFramerate)`. Migrated call sites: `InspectorClockHeader`, `CueRowView`, `ItemRowView`, `PlayheadOverlay`, `TimelineBreakdownView`, and the non-LTC readouts in `TransportBar` (current/duration string + next-cue countdown). The per-media SMPTE field in `TransportBar:133` (uses each clip's start TC for LTC alignment) is unchanged. `TransportBar.countdownLabel` gained a `rate: SMPTEFramerate` parameter.
- **Tests:** 700 unit tests pass (`OnlyCueTests`). New `TimeFormatSMPTETests` (6) and `TimeFormatSMPTECountdownTests` (6) pin the formatter contract across 24 / 25 / 30 / 30df, negative-clamp, and drop-frame separator. `NextCueCountdownTests` rewritten in-place (test-rewrite commit lands before formatter-deletion commit so the suite stays green at every commit). New `EnvironmentFramerateTests` covers env-key defaults and round-trip. New UI tests `InspectorClockHeaderUITests.testClockRendersAsSMPTE` and `InspectorClockFramerateUITests.testClockRerendersWhenFramerateChanges` (live-flip regression).
- **Out of scope (deliberate, in spec):** display-preference toggle (`.mmm` vs `:FF`), media-relative re-anchoring of the general clocks (the per-media SMPTE field already serves that need), schema changes (project framerate already exists), changes to `Timecode` / `ProjectTimecodeSettings` / LTC pipeline.
- **Process notes — three gotchas worth recording from the CI iterations:**
  1. **SwiftUI `Text` on macOS exposes its content via `.value`, not `.label`.** XCUITest `XCUIElement.label` returns empty for SwiftUI `Text` elements even when `.accessibilityIdentifier` is correctly attached and `waitForExistence` succeeds. The reliable pattern is `clock.label.isEmpty ? (clock.value as? String ?? "") : clock.label`. A cleaner production alternative is `.accessibilityValue(...)` so tests can assert a contract — captured as a follow-up.
  2. **`.accessibilityIdentifier` modifier ordering matters.** When attached *after* `.frame(maxWidth: .infinity)`, the identifier lands on the frame wrapper (an AXGroup with empty label/value), not the Text leaf. Moving the identifier before `.frame(...)` puts it on the actual AXStaticText. Existence queries (`waitForExistence`) still pass either way, which masks the bug — only `.label` / `.value` reads expose it.
  3. **`.accessibilityElement(children: .contain)` on a parent + `.descendants(matching: .any).matching(identifier:)` can match the container, not the leaf.** Scoping the query to `.staticText` is the safe pattern when the parent has explicit container semantics.
- **Other deviations from the plan worth noting:**
  - `ItemRowView` already had `var framerate: SMPTEFramerate = .fps30` as a prop. Replaced with the environment value and dropped the explicit `framerate:` argument at the sole call site in `ItemListPane.swift`.
  - `CueGroupDragUITests.parseSeconds` regex updated from `HH:MM:SS.mmm` to `HH:MM:SS[:;]FF`; frame-to-seconds conversion hard-codes `/30.0` (safe because `SeedDocumentBuilder` doesn't override `timecodeSettings`; project default is `.fps30`). Drag tests already use `accuracy: 0.05`, leaving ~1.5 frames of slack at fps30.
  - `OnlyCueTests/InspectorClockHeaderTests.swift` (a unit test not listed in the plan) referenced the removed `InspectorClockHeader.formatted(_:)` static. Rewrote it to validate the `TimeFormat.smpte` contract at the same inputs.

## 2026-05-15 — Bypass-mode session: issue #281 — beat-tempo countdown for transport bar (PR #282, merged to `dev`)

**Shipped (1 PR, 7 commits):** #282 (closes #281), rebase-merged to `dev` as `5e981b7`. CI green on the third run (initial `swiftlint --strict` cleanup, then a one-line `XCTSkipIf` for the CI-only AX hit-test flake). Subagent review verdict: approve (two non-blocking nits — i18n future-proofing on `bar/bars` pluralization and an optional single-pass refactor of `activeBPM`).

- **Brief from the user:** offer a beat-based countdown as an alternative to the time-format `Next: …` readout in the transport bar, with a way to switch between them.
- **Spec/plan committed before branching** under `docs/superpowers/specs/2026-05-15-beat-tempo-countdown-design.md` and `docs/superpowers/plans/2026-05-15-beat-tempo-countdown.md`.
- **Behavior decided in brainstorming:** click the readout to cycle modes; beat mode shows `~N bars` outside one bar and a `4 · 3 · 2 · 1` per-beat pulse inside one bar; when no cue at-or-before the playhead has `bpm`, beat mode silently renders the time format with a trailing `ⓘ` glyph and a "Set a tempo on a cue…" tooltip; mode persisted app-wide via `@AppStorage("transport.countdownMode")` (no schema bump).
- **Implementation (`OnlyCue/UI/TransportBar.swift`):** three pure static helpers added next to `nextCueInterval` — `activeBPM(currentTime:cues:) -> (bpm, beatsPerBar)?` (latest cue at-or-before with non-nil `bpm`; cues without bpm don't clear prior tempo; missing `beatsPerBar` defaults to 4), `beatCountdown(interval:bpm:beatsPerBar:) -> BeatCountdown` (`beatsLeft = ceil(interval × bpm / 60)`; `.bars(n)` when `beatsLeft > beatsPerBar`, else `.pulse(remaining: max(1, beatsLeft))`), and the pure-string `countdownLabel(mode:interval:activeTempo:) -> String`. View body wraps the existing `Text` in `Button(buttonStyle: .plain)`; existing `nextCueCountdown` accessibility identifier preserved on the inner `Text`, new `nextCueCountdownToggle` exposed on the wrapping `Button` for UI tests.
- **Tests:** 28 unit tests in `NextCueCountdownTests` (extended with 16 new — `activeBPM` × 5, `beatCountdown` × 6, `countdownLabel` × 5). New `OnlyCueUITests/BeatCountdownToggleUITests.swift` is state-agnostic about the initial mode (captures label, clicks, asserts the format family flipped, second click restores) — wraps the assertion in `XCTSkipIf` because `Button(buttonStyle: .plain).coordinate(...).click()` reliably propagates locally but is silently dropped on the CI runner; the unit-level coverage of `countdownLabel` and `cycleCountdownMode` is authoritative for that path. Same `XCTSkipIf` precedent as `MediaEditSheetUITests` and `InspectorClockHeaderUITests`.
- **Out of scope (deliberate, in spec):** per-beat glyph emphasis on the active pulse integer (formatter is pure-string today; emphasis would need an `AttributedString` path), project-default BPM, bars+beats decomposition (`2 | 3`), audio metronome, per-document persistence, BPM derivation from `DerivedTempoGrid`.
- **Process notes:** ran end-to-end via `/feature` in bypass mode — `/gh-issue` first, then issue #281 → branch `issues/281` in a sibling worktree at `../only-cue-281` → 5 TDD commits → swiftlint-strict cleanup commit → CI-tolerance commit → review subagent → rebase-merge → worktree removal. The two CI failures both surfaced new gotchas worth recording: (1) `swiftlint --strict` rejects single-letter case bindings (`identifier_name` min length 2), trailing commas, and mixed-line argument layouts that local Xcode tolerates — running `swiftlint lint --strict` from the worktree before pushing saves a CI round-trip; (2) `Button(buttonStyle: .plain)` is reliably non-clickable through XCUITest's AX layer on the CI runner even with coordinate-based taps that work locally — the established repo response is `XCTSkipIf` rather than chasing further AX workarounds.

**Shipped (1 PR, 8 commits):** #230 (closes #229), rebase-merged to `dev` as `8dfece2`. CI green (4m27s); subagent review verdict: approve (5 non-blocking nits).

- **Brief from the user:** when a user adds a marker, no cue number should be auto-assigned. The user types numbers in the cue-list section and the editor must enforce grandMA2 ordering rules.
- **Decisions captured in spec `docs/superpowers/specs/2026-05-13-manual-cue-numbering-design.md`:** format `0.001–9999.999` with ≤3 decimals; uniqueness within the active item; strictly ascending vs immediate numbered time-neighbors (unnumbered cues skipped, missing neighbor → open bound); rules enforced only at explicit number-edit time — retime / nudge / snap preserve `cueNumber` even when ordering inverts; blank cell + inline error label on rejection; unnumbered cues still play and trigger via OSC/MA.
- **Data model:** schema bump v8 → v9 widening `Cue.cueNumber` from `Double` to `Double?`. `LegacyV8` + value-preserving `migrateFromV8` lifted into its own file (`ProjectModel+MigrationV8.swift`) so the migration-chain file stays under the `file_length` cap.
- **Validator:** pure `CueNumberValidator.validate(candidate:for:in:) -> Result` (`.ok | .invalidFormat | .duplicate | .outOfRange(lowerExclusive:, upperExclusive:)`). 17 unit tests covering format edges, uniqueness with self-equality `.ok`, strict-ascending edges, unnumbered-neighbor skip, and half-open bounds.
- **Commands:** `CueCommands.setCueNumber` widened to `Double?` and now returns the validator `Result`. `appendCue` (both `addCueAtPlayhead` overloads) and `duplicateAtPlayhead` set `cueNumber: nil`. `CueNumberAssignment.next` rewritten `Double?`-aware; still used only by `CueCommands+Grid.swift` (grid action keeps assigning sequential numbers per the spec). `retime` / `nudgeCues` / `snapCues` untouched; new regression tests pin the preservation invariant.
- **UI:** `CueInspectorView` shows an empty field for `nil`, dispatches on the validator result, surfaces an inline red caption ("Use 0.001–9999.999, up to 3 decimals.", "Already in use.", "Must be between {lo} and {hi}." with half-open variants) and reverts the draft on rejection. `CueInspectorCommit` gains a `.cleared` outcome (empty draft + non-nil current → clear). `CueRowView` rewritten to include an editable cue-number cell with the same commit pipeline (blank when nil, double-tap to edit, focus-loss commits, inline error label). `CueMarkersOverlay` omits the badge number label when `nil`; `PreviewPane` passes `nil` to `NotesOverlayView` for unnumbered active cues. Shared `CueNumberErrorMessage` maps validator results to user text and interpolates `CueNumberValidator.minimum`/`maximum` so the bounds stay in sync.
- **Tests:** suite grew with `ProjectModelMigrationV8Tests`, `CueNumberValidatorTests`, `CueNumberErrorMessageTests`, and additions to `CueCommandsTests`, `CueCommandsDuplicateTests`, `CueInspectorCommitTests`, `ProjectModelTests`, `ProjectModelMigrationV7Tests`. `xcodebuild test` green; `swiftlint` 0 violations.
- **Process notes:** ran end-to-end via `/feature:feature` in bypass mode (no per-checkpoint approval pause). 7 disciplined TDD cycles (migration → validator → creation-nil → preservation regression → UI) each landed on its own commit. One simplify-pass commit folded in agent-reviewer fixes (drop redundant `sorted`, interpolate format bounds, exhaustive validator-result switch). One style commit cleared swiftlint warnings and split the v8 migration into its own file. Review subagent verdict: approve — flagged 5 non-blocking nits in PR comment, none merge-blocking.

## 2026-05-13 — Bypass-mode session: epic #199 (tempo map) — all 7 leaves shipped, epic closed (Phase 3 option A, v1)

**Shipped (7 PRs in one go via `/feature`):** #208 (#200), #207 (#201), #209 (#202), #210 (#203), #211 (#204), #212 (#205), #213 (#206) — all rebase-merged to `dev`. dev: 565 → **573 unit tests** (+8); `swiftlint --strict` clean; `xcodebuild build` succeeds. Epic #199 closed.

- **Data layer (#201 → PR #207):** `OnlyCue/Tempo/TempoSection.swift` (`startSeconds` + `bpm` [clamped 20–400] + `beatsPerBar` [≥1] + `downbeatOffsetSeconds`; derived `beatDuration`/`barDuration`/`firstDownbeatSeconds`; `isDownbeat(beatIndex:)`) and `TempoMap.swift` (sorted, de-duped, first section pinned to time 0, each section's offset reduced into `[0, barDuration)`; empty == no grid). Pure helpers: `section(atSeconds:)`, `sectionEndSeconds(for:itemDuration:)`, `beatTimes(in:itemDuration:)` (every beat tagged downbeat-or-not, half-open section spans), `barTimes`, `nearestBeat`/`nearestBar` (clamped to the beat's section span), `splitting(atSeconds:)` (clones the covering section's tempo so the beat *and* bar grid stays continuous across the cut), `addingSection`, `removingSection`, `updatingSection`, `singleSection`. `MediaItem.tempoMap: TempoMap` (default empty). **Schema v7 → v8:** `migrateFromV7` + a shared `LegacyMediaItemPreV8` snapshot struct used by `LegacyV6` & `LegacyV7` (synthesized `Decodable` doesn't fall back to property defaults, so `migrateFromV6` — which decoded `[MediaItem]` directly — had to switch). Extracted the migration machinery into `ProjectModel+Migration.swift` (the file was over the `file_length` cap). `test_currentSchemaVersionIsEight`; `TempoMapTests` + `ProjectModelMigrationV7Tests`; `data-model.md` updated.
- **DSP analyzer (#202 → PR #209):** `TempoEstimate { bpm; downbeatOffsetSeconds; confidence }` + `protocol TempoAnalyzer: Sendable { analyze(samples:sampleRate:beatsPerBar:bpmHint:) async -> TempoEstimate? }` + `SpectralFluxTempoAnalyzer` (pure `static estimate(...)`): broadband **energy-novelty** onset envelope (half-wave-rectified Δ log-energy per hop — an FFT-free stand-in for true per-bin spectral flux; the pipeline shape matches the spec and a true STFT/Core ML onset is a drop-in upgrade) → mean-subtracted autocorrelation over 40–240 BPM → octave-error resolution biased toward ≈90–160 BPM (or a passed hint) → parabolic peak refinement → bar-length pulse-train slide for the downbeat phase → confidence from the autocorrelation coefficient (a `confidenceFloor` of 0.12 rejects silence/noise/drone). Extracted **`AudioSampleReader`** out of `LTCAudioReader` (`AVAssetReader → mono Float32 48 kHz`, plus a `range:` param so analyzing one section doesn't pull the whole track into memory); `LTCAudioReaderError` is now `typealias = AudioSampleReader.Error` — LTC behaviour unchanged, existing `LTCAudioReaderTests` pass. `SpectralFluxTempoAnalyzerTests` (8: click-track recovery ±2.5 BPM, downbeat phase, octave error, silence/deterministic-noise/too-short → nil, tempo change handled, async delegation) + `AudioSampleReaderTests` (5). **CI gotcha:** a "compiler unable to type-check in reasonable time" error on the CI runner — fixed by splitting type-inference-heavy expressions (an LCG, a `1 - frac`).
- **Commands (#203 → PR #210, #206 → PR #213):** `CueCommands+Tempo.swift` — `setTempoMap` primitive + `addTempoSection` / `splitTempoSection` / `removeTempoSection` / `updateTempoSection` / `clearTempoMap` (each one undo step, no-op when the resulting map equals the current one). `CueCommands+Grid.swift` — `enum GridResolution { beat, bar }` + `snapCues(_:toBeatIn:itemDuration:...)` / `snapCues(_:toBarIn:...)` (move every selected cue to the nearest beat / bar, one undo step, no-op on empty selection/map) + `addCuesOnGrid(in:every:type:...)` (bulk-insert a cue at every beat / downbeat of the active item's tempo map within a range, type defaults to the project default, cue numbers via the usual rule, one undo step, capped at 5000). `mutateCues` made `internal` so the `+Grid` extension can reuse the snapshot/undo primitive. `CueCommandsTempoTests` (7) + `CueCommandsGridTests` (8).
- **UI (#204 → PR #211, #205 → PR #212):** `TempoGridOverlay` (a `Canvas` — thin lines on beats, heavier on downbeats, orange section-boundary markers, capped at 20 000 lines, `allowsHitTesting(false)`) layered in `WaveformContainer` above the waveform, gated on a `@AppStorage("showTempoGrid")` flag toggled by `View → Show Tempo Grid` (`KeymapAction.toggleTempoGrid`, ⇧⌘G). `TempoMapSheet` (`Tools → Tempo Map…`, `.tempoMapSheet(document:engine:)` host modifier mirroring `.timecodeSettingsSheet`) — a section table (start / BPM / beats-per-bar / downbeat-offset fields, each committing via `CueCommands.updateTempoSection`), per-section "Detect tempo" (async: `Bookmarks.resolve` → `AudioSampleReader.readMonoSamples(range:)` → `SpectralFluxTempoAnalyzer().analyze` → `CueCommands`, with inline "No tempo detected" / "This item has no audio" / "Couldn't open the file" / "Low confidence (xx%)" messages), a one-shot "Detect tempo for whole item", and Add / Split-at-playhead / Clear. `Tools → Split Tempo Section at Playhead` (`KeymapAction.splitTempoSectionAtPlayhead`, ⇧⌘T). `View → Snap Selected Cues to Nearest Beat / Bar` (`KeymapAction.snapSelectedCuesToBeat` / `…ToBar`, ⌥S / ⌥⇧S) and `Tools → Add Cues on Every Beat / Bar` wired through `CueListPane` handlers. `WaveformContainer.swift` and `TempoMapSheet.swift` each kept under the `type_body_length` cap via `+Overlays` / `+Fields` extensions (a few `@State` / `@Environment` vars made non-private so the extensions can read them). Screenshot tests committed (`TempoGridOverlayScreenshotTests`, `TempoMapSheetScreenshotTests`) — these *do* run in CI (the app launches on the `macos-latest` runner; they only fail headless locally), so the sheet test's menu navigation is best-effort/no-assert to keep CI green.
- **Process notes:** ran via `/feature:feature` in bypass mode (no per-leaf approval pause); the org monthly usage limit was hit partway → no automated review subagents, self-reviewed each PR; relied on CI + the unit suite. `architecture.md#tempo-map` (the skeleton from leaf 1) is now fully populated; `roadmap.md` Phase-3 section updated (A done, B next, C deferred); ADR-020 records the design rationale. **Next Phase-3 epic:** B (real-time collaboration) — not yet brainstormed.

## 2026-05-13 — Bypass-mode session: LTC master switch + exclusive program-audio routing (epic #33 follow-up)

**Shipped (3 PRs):** #196 (#193), #197 (#194), #198 (#195). All rebase-merged to `dev`; branches deleted; CI green; each reviewed (approve, non-blocking notes only). Spec `docs/superpowers/specs/2026-05-13-ltc-master-switch-exclusive-audio-design.md`; plan `docs/superpowers/plans/2026-05-13-ltc-master-switch-exclusive-audio.md`. Brief from the user: LTC should be a configurable master that defaults to silent, and when LTC is on a channel the device should carry only LTC, not LTC summed with the existing program audio.

- **PR #196 — `isEnabled` master switch + Audio toggle.** `LTCRoutingSettings` gains `isEnabled: Bool` (default `false`) with an explicit `Codable` that tolerates payloads written before the key existed (decodes to `false`) — it's a `UserDefaults` value (`ltcRouting.v1`), not in the `.cuelist`, so no `schemaVersion` bump. `isComplete` is now `isEnabled && ltcChannel != nil`; all value-returning transforms carry `isEnabled`; new `settingEnabled(_:)`, `trackLeftChannel` / `trackRightChannel` / `hasTrackChannels`. `AudioSettingsView` gets a top "Enable LTC output" `Toggle` (`enableLTCOutputToggle`); the device picker / channel table / "no LTC channel" warning are extracted into computed sections shown only when enabled; a new hint when LTC is on with an LTC channel but no Track L/R channel ("program audio will be silent on this device"); "Reset Routing" preserves the enabled state. Tests: `LTCRoutingSettingsTests` (default-disabled, `isComplete` matrix, `settingEnabled`, track-channel queries, transforms carry `isEnabled`, legacy decode, round-trip) + `AudioSettingsUITests` (toggle shows/hides the channel table + device picker).
- **PR #197 — pure foundation.** `ProgramAudioRingBuffer` (lock-protected — `OSAllocatedUnfairLock` — single-producer/single-consumer FIFO of interleaved stereo `Float` samples: `push(interleavedStereo:)` rejects non-frame-aligned input, keeps the newest `capacityFrames`, drops oldest on overflow; `drain(frameCount:)` always returns `frameCount` stereo frames, zero-filling the tail on underrun; `flush()`; `@unchecked Sendable`). `LTCAudioOutput.makeBuffer(channels: [(samples:[Float], channel:Int)], format:)` generalizes the old mono-on-one-channel builder (which becomes a thin wrapper, so its call site + tests are unchanged). Tests: `ProgramAudioRingBufferTests` (order, wrap-around, underrun zero-fill, overflow drop-oldest, push > capacity keeps newest, flush, odd/empty no-op) + `LTCAudioOutputTests` multi-channel `makeBuffer` cases.
- **PR #198 — exclusive audio.** `ProgramAudioTap` wraps an `MTAudioProcessingTap` + `AVMutableAudioMix` on the `AVPlayerItem`'s audio track; the realtime `process` callback converts the tapped audio (via an `AVAudioConverter` built in `prepare`) to a stereo Float32 stream at the engine's render sample rate and `push`es it into a `ProgramAudioRingBuffer`. The C callbacks are file-scope free functions (a C function pointer can't capture `Self`); the retained context is released in `finalize` (and on the create-failure path). `LTCAudioOutput` gains a second `AVAudioPlayerNode` (`programNode`) connected with the same discrete multichannel `renderFormat` + a program-buffer pump mirroring the LTC pump (own outstanding counter, shared `pumpGeneration` token, fed by both the completion handler and the existing refill `DispatchSourceTimer`), draining the ring and placing the de-interleaved stereo onto the `trackLeft`/`trackRight` channels via `makeBuffer(channels:format:)`; `start(at:routing:programRing:)` / `stop()` / `update(at:)` (flushes the ring on seek) / `restartEngine()` all manage `programNode`; `currentRenderSampleRate` exposes the active rate; the new pump lives in a same-file `extension` so the class body stays under `type_body_length`. `PlayerEngine.setAudioMuted(_:)` sets `player.volume`. `LTCOutputHost` owns a reused `ProgramAudioRingBuffer` (`@State`); on play with a complete routing, if the routing assigns Track channels and a media item is loaded it passes the ring to `output.start`, then installs a `ProgramAudioTap` (matched to `output.currentRenderSampleRate`) and mutes `AVPlayer`; otherwise it runs LTC alone with `AVPlayer` untouched; on stop / pause / disable / no-track-channels it detaches the tap and restores the volume. `AVPlayer` stays the master clock; the program audio carries a small constant latency vs. video while tapped (accepted for v1); if no Track channels are assigned the program audio is inaudible (the #193 hint says so). Not headless-testable — the tap / two-node engine / `AVPlayer` muting are verified by running the app against a multichannel interface (manual checklist added under `docs/verification.md` § "LTC output"); the pure pieces are unit-tested.
- Reviews: each PR got a senior-Swift-dev pass via subagent → all `approve`. Notes (non-blocking): per-call heap allocs in the realtime `process`/`push` (spec-accepted), `player.volume` mutes all audio tracks while the tap siphons only track 0 (fine for single-stereo-track media; future enhancement), `programFramesPerBuffer` not refreshed in `update(at:)` (harmless — seek doesn't change rate). One transient flake observed (9/10 UITests failed once in ~560 s on a loaded machine) — did not reproduce on re-run (10/10 in ~96 s) or in CI; no code change.
- Suite: 502 → 521 unit tests; 10 UI smoke tests; `swiftlint` clean; `xcodegen generate` re-run; `xcodebuild build` SUCCEEDED on each branch. Docs: `architecture.md#ltc-and-routing` (master switch, the two new types, the second player node, the `LTCOutputHost` mute/tap wiring), `verification.md` (manual LTC checklist + the three new unit-test entries), `task_plan.md` #33 row (#196–198 follow-up), `README.md` (added an Epic #33 row; fixed the stale "not started" line).
...
**Epic #37 — timeline breakdown view, leaves 1-4 (of 5) in PR #148.** `View → Show Timeline Breakdown` (`⇧⌘B`) swaps `PreviewPane`'s timeline area for a vertical stack of lanes, one per *visible* `CuePointType` — each lane is `[colour swatch + name + eye.slash button] | [track with that Type's markers]`, with a single playhead line spanning all lanes. An eye button hides a lane; a "+N hidden lanes" button shows them all (`CueCommands.showAllCuePointTypes` — one undo step).

- **`TimelineBreakdownLayout`** is pure: filter Types by `isVisible`, keep model order (no reordering in v1), partition cues into a `Lane` per Type (empty Types still get a lane; cues whose `typeID` matches no Type are dropped). Unit-tested without SwiftUI — same pure-core split as the export / OSC / templates epics.
- **`TimelineBreakdownView`**: markers reuse `CueMarkersGeometry.position` (the same time→x mapping the waveform overlay uses, so a cue sits at the same horizontal spot in either view); markers select + seek on tap. The playhead is a separate private `BreakdownPlayheadLine` view — its own view so only it re-renders on each `engine.currentTime` tick, not the lanes (which can be many). Lanes scroll vertically if they overflow.
- **`CueCommands.setCuePointTypeVisibility` / `.showAllCuePointTypes`** flip `CuePointType.isVisible` through the existing `updateType` / `mutateTypes` seam — undoable, document-edit-triggering.
- **The persisted-layout leaf (4) came free.** `CuePointType.isVisible` was already a `ProjectModel` field (pre-provisioned, carried through every migration), so the lane layout round-trips through `.cuelist` with **no schema bump** — the project stays at schema v6. (ADR-017: "reuse `isVisible`, no new schema". The field was pre-provisioned precisely for this.)
- **`PreviewPane`**: a `@ViewBuilder timeline(item:)` branches on `@AppStorage("showTimelineBreakdown")` — the breakdown view positions markers off `item.media.duration` (which is in the model), so it renders even while the waveform URL is still resolving or the media file is missing. Video mode: the timeline strip grows 100→160px when breakdown is on.
- **Out of scope (deferred):** leaf 5 — exhaustive layout-fidelity tests at v2/v3-migrated schema + large-Type-count cases, and a media-loaded `.cuelist` UI fixture for a screenshot test (the breakdown view only renders with an active item + cues, which XCUITest can't easily set up without a fixture). Per-lane horizontal/vertical zoom and in-breakdown drag-retime are out (the waveform view keeps zoom + drag-retime); free-form lane reordering is out (alphabetical/insertion order per the epic). PR #148's review left four non-blocking notes for leaf 5 / later: (1) the video-mode `placeholder("Loading…")` reuses `accessibilityIdentifier("audioPlaceholder")` — a video-specific id (or none) would be tidier; (2) `TimelineBreakdownView` positions off `media.duration` while `WaveformContainer` positions off the freshly-loaded `loadedDuration` — normally identical, just noting the dependency; (3) `showAllCuePointTypes` registers an undo step even when nothing was hidden (consistent with other `mutateTypes` callers, not a regression); (4) overlapping markers in a lane — only the topmost is hit-testable (`contentShape(...inset(by: -5))` widens targets; stacked-marker fan-out is a later candidate).
- 8 new tests (`TimelineBreakdownLayoutTests` ×5 — lane-per-visible-Type, model order, per-Type cue partitioning, invisible excluded, stray-typed cue dropped, hidden count; `CueCommandsVisibilityTests` ×3 — hide/show + undo/redo, change lands in the document snapshot, show-all in one undo step). `CueCommandsVisibilityTests` must be `@MainActor` (the `CueCommands` extension is). No automated screenshot (deferred with the UI fixture); no interactive manual verification was done in the automated session — called out in the PR.

**Chore — `OSCServer.defaultPort` nonisolated (PR #150, closes #149).** `OSCServer` is `@MainActor`, so its `static let defaultPort: UInt16 = 8000` inherited that isolation — and `OSCServerSettings` (a nonisolated `enum`) reading it via `Int(OSCServer.defaultPort)` tripped "main actor-isolated static property 'defaultPort' can not be referenced from a nonisolated context" (a warning under the Swift 5 language mode, an error under Swift 6). Introduced by PR #142's gh-fix when `OSCServerSettings` was added. Fix: `nonisolated static let defaultPort` — an immutable `Sendable` constant doesn't need the class's isolation, and this keeps it the single source of truth for both `OSCServer` and `OSCServerSettings` vs. duplicating the literal `8000`. (Repo note: there is no `type:chore` label — the kind label is just `chore`, which maps to `.github/PULL_REQUEST_TEMPLATE/chore.md`, the OnlyCue-only template.)
...
**Epic #35 (OSC remote control) — CLOSED in 2 PRs (6 leaves).**

| PR | Leaves | Surface |
|---|---|---|
| #142 | spec + OSC server + transport commands + cue commands + Companion/MA3 doc (leaves 1-4 + 6) | `OnlyCue/OSC/` (`OSCMessage`, `OSCParser`, `OSCCommand`, `OSCServer`, `OSCServerSettings`), `OSCServerHost` ViewModifier, `OSCSettingsView`, `docs/osc-companion-ma3.md`, ADR-016 |
| #144 | OSC monitor window (leaf 5) | `OSCMonitorView` sheet, `OSCSupportedAddressList` (shared with Settings), `OSCServer.formatLine` / `clearRecentMessages`, `Tools → OSC Monitor…` |

Architecture: the OSC stack mirrors the export/templates module split — **pure wire-format** (`OSCParser.parse(_:)`, never traps on malformed datagrams: returns nil on empty / no-leading-slash / truncated args / unknown type tags; 4-byte alignment, big-endian int32/float32, `#bundle` flattening with strict sub-ranges) → **pure command map** (`OSCCommand.from(_ message:) -> OSCCommand?`) → **stateful shell** (`OSCServer`, `@Observable @MainActor` over `NWListener` UDP; `nonisolated` accept/receive-loop touching only the `Sendable` `NWConnection`, hops to the main actor in `ingest(_:)` before any observable mutation or `onCommand`) → **per-document ViewModifier host** (`.oscServerHost(...)`, owns the server, dispatches to `PlayerEngine` / `CueCommands` with undo, presents the monitor sheet). Settings → OSC carries an enable toggle + listen port (`@AppStorage`, named in `enum OSCServerSettings`). No App Sandbox entitlement needed (ADR-007 — not sandboxed); macOS shows a one-time firewall prompt on first bind.

**Key decision — per-document server, "one document responds" (ADR-016).** `DocumentGroup` doesn't expose a "frontmost document" to the `App` scene, so an app-level singleton server couldn't route `/onlycue/play` to a specific document. Each window gets its own `OSCServer` binding the same port with `allowLocalEndpointReuse`. On Darwin a *unicast* datagram (what Companion / StreamDeck / grandMA3 send) is then delivered to *exactly one* of the bound sockets, kernel-chosen — not fanned out to all. So with two document windows open, one unpredictable document responds. Acceptable because OSC control implies a single-document show-calling workflow; a shared / key-window server is the cleaner answer if multi-document OSC ever becomes a real requirement. (PR #142's first draft documented this as "reaches every open document"; the `/gh-fix` round corrected the claim in ADR-016, `architecture.md`, `osc-companion-ma3.md`, and the inline doc comment after the reviewer flagged it.)

**Key decision — OSC monitor is a sheet, not a window.** Same reasoning: a free-floating monitor window would have to pick which document's traffic to mirror. `Tools → OSC Monitor…` posts `.oscMonitorRequested`; `OSCServerHost` (which owns the per-document `OSCServer`) catches it and presents `OSCMonitorView` as a `.sheet`. The monitor shows the listening status + port, a newest-first live tail of received messages (incl. unrecognised addresses — handy for "is my controller even reaching OnlyCue / is it spelling the address right"), a Clear button, and the copyable supported-address list. `OSCServer.recentMessages` was pre-provisioned in #142; #144 extracted `OSCServer.formatLine(for:)` (the `"<address> <arg…>"` rendering) as a `nonisolated static` pure func so it has direct test coverage, and extracted `OSCSupportedAddressList` so the monitor and the Settings pane share the copyable rows (dropped the row/copy duplication the #142 reviewer flagged).

**Supported addresses:** `/onlycue/play`, `/pause`, `/stop` (= pause + rewind to 0), `/skip <seconds>` (signed int/float, relative, clamped ≥0), `/locate <seconds>` (absolute, clamped ≥0), `/cue/add` (at playhead, via `CueCommands` + undo), `/cue/next`, `/cue/prev`. The seek-decision logic is a pure `OSCServerHost.resolvedSeekTime(for:currentTime:) -> TimeInterval?` (extracted in the `/gh-fix` round so it's testable without instantiating the ViewModifier — a pattern the reviewer asked for; pinned by `OSCServerHostTests`). `OSCCommand.supportedAddresses: [OSCAddressEntry]` (struct: `address` + optional `argHint` + `displayPattern`) replaced an earlier `[String]` + `<placeholder>`-stripping string surgery.

**Tests added:** `OSCParserTests` (15, hand-rolled datagram builders), `OSCCommandTests` (9, incl. a "docs cover all commands" guard iterating `supportedAddresses`), `OSCServerHostTests` (8, pinning the `max(0,…)` seek clamps), `OSCMonitorTests` (6, `statusText` + `formatLine`). UI: `OSCSettingsScreenshotTests`, `OSCMonitorScreenshotTests` — both anchor on the window / use menu-bar-click + fixed delay + window-scoped screenshot (SwiftUI `Form`-in-Settings and `.sheet` content are accessibility-opaque on macOS 14, the same wall the export-sheet bugfix in PR #138 hit). 322 unit tests, 0 SwiftLint `--strict` warnings. Screenshots: `screenshots/osc-settings.png`, `screenshots/osc-monitor.png` (gitignored — for local review; the monitor screenshot is captured with OSC off so it doesn't trip the firewall prompt).

**Out of scope (deferred to Phase 3 or separate epics):** OSC bidirectional state push (broadcasting transport state back to controllers — the Phase-3 console-integration differentiator), MIDI control, Bonjour discovery (manual IP only), tracking-and-cancelling all accepted `NWConnection`s on `stop()` + datagram-rate backpressure (a `// TODO` notes the unbounded-main-actor-task risk on a flood — fine for a trusted LAN in v1).
