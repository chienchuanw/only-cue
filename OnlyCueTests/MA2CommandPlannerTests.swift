...
final class MA2CommandPlannerTests: XCTestCase {
    ...
    private func cue(
        _ number: Double, _ name: String, time: TimeInterval, fadeIn: Double = 0, fadeOut: Double = 0
    ) -> Cue {
        Cue(
            id: UUID(), typeID: UUID(), cueNumber: number, name: name, time: time,
            notes: "", fadeTime: FadeTime(fadeIn: fadeIn, fadeOut: fadeOut)
        )
    }

    private func target(seq: Int = 900, page: Int = 1, exec: Int = 15) -> MA2PushTarget {
        MA2PushTarget(
            sequenceSlot: seq, timecodeSlot: 9, executorPage: page, executorNumber: exec,
    ...
    func test_buildsOrderedCommandList_deleteFirst_numberSorted() {
        ...
        let commands = MA2CommandPlanner.commands(
            cues: cues, target: target(), sequenceName: "Song A",
    ...
    func test_emitsOutfade_whenFadeOutPositive() {
        let commands = MA2CommandPlanner.commands(
            cues: [cue(1, "C", time: 0, fadeIn: 0, fadeOut: 2)], target: target(),
            sequenceName: "S", startTimecodeFrames: 0, framerate: .fps30
        )
    ...
    func test_stripsEmbeddedQuotes_inCueAndSequenceNames() {
        let commands = MA2CommandPlanner.commands(
            cues: [cue(1, "he said \"hi\"", time: 0)], target: target(seq: 5, page: 2, exec: 3),
            sequenceName: "a\"b", startTimecodeFrames: 0, framerate: .fps25
        )
