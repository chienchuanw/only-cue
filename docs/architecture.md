...
| Routing playback | `LTCAudioOutput` (`@MainActor ObservableObject`) — opens an `AVAudioEngine` on the routed device (`AudioUnitSetProperty(kAudioOutputUnitProperty_CurrentDevice)`, or the system default when `deviceUID == nil`), connects an `AVAudioPlayerNode` with a discrete-layout multichannel float format (`renderFormat(channelCount:sampleRate:)` — `AVAudioFormat`'s simple inits reject 3/5/7… channels), and pumps `LTCSchedule.nextBuffer()` as `makeBuffer(monoSamples:format:channel:)` (mono LTC on the assigned channel, silence elsewhere), keeping ~3 buffers scheduled ahead via the completion handler. `start(at:routing:)` / `stop()` / `update(at:)` for the transport; rebuilds on `AVAudioEngineConfigurationChange` (= survives device disconnect). **Not yet wired to the transport (leaf C)**, and the engine/device path isn't headless-testable — `LTCSchedule` / `makeBuffer` / `renderFormat` are unit-tested; the live path is verified by running the app | `OnlyCue/LTC/LTCAudioOutput.swift` |
