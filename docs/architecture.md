...
| Routing playback | *(leaf B, not built)* the `AVAudioEngine` / `AVAudioPlayerNode` path that schedules `LTCSchedule.nextBuffer()` onto the assigned channel of the routed device, driven by `PlayerEngine.currentTime`; rebuilds on `AVAudioEngineConfigurationChange` (= survives device disconnect) | `OnlyCue/LTC/` |
