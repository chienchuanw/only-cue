...
- No MIDI-specific step logic. The three control paths — `DocumentView+ShowGo`
  (keyboard + UI), `OSCServerHost`, `MIDIInputHost` — all call the same pure
  function and must keep identical semantics.
- No wrap-around, in either direction.
- No "restart the current cue" affordance is added to replace the removed
  behavior. `Prev` then `Next` does it; `Stop` covers back-to-top.
...
- `MediaItemTypeFilterTests.test_stepPrev_type_skipsOtherTypes` — from 4 with
  typeB expected 2; B candidates are [2, 5], active is 2, nothing earlier, so
  now expects nil. A third B cue is added to the reasoning so the type-filtered
  path still has a positive stepping case.
...
Scenario: type-filtered stepping stays within its type
  Given typeA cues at 1 and 3 and typeB cues at 2, 5 and 8
