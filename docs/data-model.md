...
struct MediaItem: Identifiable, Equatable {
    ...
    var ltcChannelSelection: LTCChannelSelection // since v23; which channel the user says carries LTC (#793)
}
...
struct FadeTime: Codable, Equatable, Hashable {
    var fadeIn: TimeInterval      // seconds; 0...FadeTime.maximum (3600) at both trust boundaries
    var fadeOut: TimeInterval     // seconds; same range
    // .symmetric(t) → FadeTime(fadeIn: t, fadeOut: t)
    // Canonical string form: "1.5" when fadeIn == fadeOut, otherwise "1/2"
    // The memberwise init is deliberately unchecked; parse rejects and decode clamps.
}
...
| `item.ltcChannelSelection` | Since v23. `LTCChannelSelection` — which channel the user says carries LTC (`auto` / `channel:N` / `none`). Authored, unlike `rememberedLTC`; Clear and Re-detect never touch it. Encoded as a flat string; absent-means-`.auto` so untouched documents keep their existing byte shape. v22 → v23 migration leaves it `.auto` (missing key → `.auto`). |

## Versioning policy

- `schemaVersion: 23` is the current file. We will **never** mutate v23 semantics; new fields go in v24. `ProjectModel.currentSchemaVersion` is the source of truth if this line ever drifts.
...
- Migrations are pure functions `(JSONvN) -> ProjectModel`, applied during `ProjectModel.decode(from:)`. Pre-v4 chains run `assignCueNumbersBySort` so cues land with sequential `cueNumber` values; every chain backfills `fadeTime = .symmetric(0)` at the cue boundary so pre-v5 sources land with a valid `fadeTime`; every chain drops the legacy per-cue `colorHex` at the boundary so any pre-v6 source lands with color resolving via the Type; every chain seeds `timecodeSettings = .default` so any pre-v7 source lands with valid timecode settings; and every chain lands an empty `tempoMap` on every item so any pre-v8 source has a valid (empty) tempo map:
  ...
  - **v22 → current** — `ltcChannelSelection` defaults to `auto`; `rememberedLTC` gains optional `validFrom` / `validUntil`, both absent-means-unbounded, so existing readouts are unchanged.
- v23 is a one-way upgrade: every prior build (v1 through v22) cannot open v23 files.
