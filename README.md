...
**[v0.20.0](https://github.com/chienchuanw/only-cue/releases/tag/v0.20.0) is the latest release** — **hear only the music from a file that has timecode striped on it**. When an imported audio file carries **LTC (linear timecode) on its own channel**, OnlyCue now **auto-mutes that channel** so you hear only the music (the surviving music is centered to both ears), and the **waveform draws the music track only** instead of the timecode buzz — with a small **badge** showing the detected start timecode. A per-clip **Source audio: Music only / Original (with timecode)** toggle (in the Edit Media sheet and the clip's right-click menu) switches a clip back to the raw file for verification, and that choice is **remembered in the project** (schema v19). Files with no striped LTC are untouched. Builds on v0.19.0's timecode-from-imported-audio scan.
