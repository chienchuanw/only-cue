...
| Media reader | `LTCAudioReader.readMonoSamples(from url:) async throws -> [Float]` — first audio track → linear PCM, mono, float32, 48 kHz (keeps the decoder's bit-period maths integer-ish for 24/25/30); `LTCAudioReaderError` (`.noAudioTrack` / `.readerFailed`). `decodeTimecodes(from url:)` runs it straight through `LTCDecoder`. Round-trips with a `LTCFrameStream`-written WAV in tests | `OnlyCue/LTC/LTCAudioReader.swift` |
| Striped-LTC drive | *(leaf 5, not built)* feed `LTCAudioReader.decodeTimecodes(...)` into `PlayerEngine` so an imported file's striped LTC drives the timeline (the generator can override it) | `OnlyCue/LTC/`, `OnlyCue/Media/` |
