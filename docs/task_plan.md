...
| [#6](https://github.com/chienchuanw/only-cue/issues/6) | E4 video preview — `AVPlayerLayer` via `NSViewRepresentable` | ⏭️ next |
...
5. **#6 (E4 video preview)** — `AVPlayerLayer` wrapped via `NSViewRepresentable` (`PreviewPane`/`VideoPreviewView`). Audio docs render the existing static placeholder; video docs render frames driven by the same `PlayerEngine`. Smallest seam between AppKit and SwiftUI in the codebase so far — be careful with layer-backed view lifecycle.
6. **#7..#11 (E5..E9)** — feature epics in build-sequence order. Leaves expanded JIT.
7. **#13 (C3) → #12 (E10)** — release pipeline, then ship.
