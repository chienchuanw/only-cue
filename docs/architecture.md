...
The cue selection is a `Set<Cue.ID>` (`DocumentView.cueSelection`, not undoable, cleared on item switch) — `CueListPane`'s `List(selection:)` gives Cmd-click / Shift-click multi-select natively, and `CueMarkersOverlay` / `PreviewPane` / `TimelineBreakdownView` highlight every cue whose id is in the set (`selectedCueIDs`). A plain marker click *replaces* the selection (`cueSelection = [id]`); a ⌘- (or ⇧-) marker click *toggles* that cue in/out (`onToggleCue` → `formSymmetricDifference`; the modifier state is read via `NSEvent.modifierFlags` in the tap handler — Shift-*range* on markers isn't tracked, the list has it natively). `S` snap-to-playhead and Option+←/→ nudge act on the **whole** selection in one undo step (`CueCommands.snapCues(_:to:…)` / `CueCommands.nudgeCues(_:by:…)`, both built on `mutateCues`); duplicate-at-playhead stays single (`CueListPane.soleSelectedID`) — duplicating N cues at one point would just stack them, as would snapping them, but the latter is the literal meaning of the command. While the set isn't exactly one cue the inspector shows its empty state and seek-on-select / scroll-to-row is suppressed; delete (`⌫` / `onDeleteCommand`) removes every selected cue. The waveform's transient inspect-magnifier (vertical zoom) and horizontal pinch-zoom are unrelated view gestures (epic #36, shipped). With this, epic #36's leaves are all done (the gain-control leaf was dropped by the owner — the existing pinch + magnifier zoom cover it).
