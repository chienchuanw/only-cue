...
**[v0.11.0](https://github.com/chienchuanw/only-cue/releases/tag/v0.11.0) is the latest release** — a downloadable DMG with two workflow improvements: in **Show mode**, GO / prev-cue / next-cue (button, Return, or OSC) can be filtered to a single **cue type**, with the current-cue highlight and notes following the filter and other-type cues dimmed in the list; and the **Timeline Breakdown**'s hidden-lane footer is now a menu that re-shows a single lane at a time (or all).
