...
- **Cue List Pane** (`CueListPane`, ID `cueListPane`) — right pane. Stacks the following:
  ...
  - **Cue List** — filterable list of cues for the active item with a leading color stripe and columns `#`, Name, Fade, Info (in that order); rows are `CueRowView`. The Time column was removed in #661; Fade returned as an editable column between Name and Info in #804. Includes the optional **BPM Column** (cue-anchored tempo). When no cues exist, shows the **Cue List Empty State**.
  - **Cue Row Interaction** — a single click in the `#`, `Name`, or `Info` column puts the caret in that field, and editing never moves the playhead. The leading colour stripe is the row's handle: clicking it selects the cue and seeks to its time. Holding a selection modifier anywhere on the row selects instead of editing or seeking, and the two modifiers mean different things: `⌘`-click toggles that one row in or out, `⇧`-click selects the contiguous run from the anchor (the last row clicked without `⇧`) to the clicked row, replacing the selection. `⌘⇧` is a `⇧`-click — adding a range to an existing selection is not supported. Show mode locks the three columns but keeps the stripe live.
