...
C1 bootstrap, C2 CI, E1 skeleton, E2 player core, E3 media import, E4 video preview, E5 waveform, E6 cue list pane, E7 add/edit/delete cues, and E8 cue markers have shipped ([#14](https://github.com/chienchuanw/only-cue/pull/14)–[#23](https://github.com/chienchuanw/only-cue/pull/23)). The waveform now shows draggable colored markers for each cue: tap a marker to seek, drag it to retime (single undo step), and the overlay shares the waveform's pre-padded frame so positions stay aligned. Every mutation still flows through `OnlyCue/Commands/CueCommands.swift` and is undoable (⌘Z / ⌘⇧Z). Next up: [#11](https://github.com/chienchuanw/only-cue/issues/11) (E9 polish — empty states, missing-media relink alert, app icon, default keyboard shortcuts, About box). Track everything on the [issue board](https://github.com/chienchuanw/only-cue/issues).
