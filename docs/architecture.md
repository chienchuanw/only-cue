...
| Frame stream | `LTCFrameStream` (`startTimecode` + `sampleRate` + `amplitude`) — `samples(frameCount:)` yields a seamless `[Float]` for N consecutive frames (biphase polarity carried across the joins via `LTCEncoder`'s `endLevel`), `timecode(atFrameOffset:)`, `samplesPerFrame`. Pure value type — the engine that schedules these buffers is separate | `OnlyCue/LTC/LTCFrameStream.swift` |
...
| Buffer plan | `LTCSchedule` (`startTimecode` + `sampleRate` + `framesPerBuffer` + `amplitude`) — maps a sequential buffer index to a `Buffer` (`{ index, timecode, samples }`): `timecode(forBufferIndex:)`, `samples(forBufferIndex:)` (a `LTCFrameStream` run of `framesPerBuffer` frames), `nextBuffer()` (mutating, sequential), `samplesPerBuffer` / `bufferDuration`, `targetBufferCount(elapsedSeconds:leadBuffers:)` (how many buffers the engine keeps emitted to stay ahead), `framesPerBuffer(forTargetSeconds:rate:)`. Pure — `LTCAudioOutput` owns one and pumps `nextBuffer()` onto an `AVAudioPlayerNode` | `OnlyCue/LTC/LTCSchedule.swift` |
