...
| **Epic [#40](https://github.com/chienchuanw/only-cue/issues/40) — custom keyboard shortcuts** | **In progress (~4 of 5 leaves).** A pure `Keymap` (`KeymapAction` × `KeyChord`) persisted as a sparse, forward-tolerant JSON object under `keymap.v1` via `KeymapStore`, with advisory conflict detection (ADR-018). Settings → Keyboard tab lets you rebind any menu command — click a row, press a new chord, Esc cancels — with per-row reset-to-default, conflict ⚠︎, and Reset All; `AppCommands` reads the keymap. Remaining: the document-window shortcuts (`m`, `0`–`9`, Space, arrows) read the keymap + show as editable rows. |
...
- **Phase 2 — Pro handoff** — nine epics filed; #32, #34, #35, #37 and #39 complete; #36, #38 and #40 in flight. Open epic not yet started: [#33](https://github.com/chienchuanw/only-cue/issues/33) LTC.
