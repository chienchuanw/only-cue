...
**[v0.21.0](https://github.com/chienchuanw/only-cue/releases/tag/v0.21.0) is the latest release** — **rearrange the workspace and save named layouts**. The document window's three panes are now **resizable and collapsible per editor mode**: drag the **inspector divider** (or `⌥⌘I` to hide/show it), resize or collapse the **sidebar**, and each of Cue / Lyric / Show remembers its own arrangement. Save any arrangement as a named **workspace preset** from **View ▸ Workspace** (with Save As…, Overwrite, Rename/Delete, and Reset to Default), applied to the frontmost window; a preset is a snapshot, so dragging a divider never silently rewrites it, and layouts survive relaunch. All without changing the document format — layouts live in app preferences, not the `.cuelist`.
