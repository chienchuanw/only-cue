...
SMPTE Linear Timecode generation synced to playback (epic #33), routable to a configurable Core Audio output. **Built so far:** the timecode value model + the 80-bit LTC frame + the biphase-mark modulation primitive + the sample synthesis (rows 1–5 below); the project's persisted timecode settings (`ProjectModel.timecodeSettings` — `ProjectTimecodeSettings`, schema v7); and the **Timecode Settings sheet** (`Tools → Timecode Settings…`) that edits them (`CueCommands.setProjectTimecodeSettings`, undoable; text parsed by `Timecode.parse(_:rate:)`). **Not yet:** Core Audio output-device picker + per-channel routing (LTC vs Track L/R) + the `AVAudioEngine` playback path fed by `PlayerEngine.currentTime`, striped-LTC playback (read existing LTC off imported media), and the Audio side of the preferences UI (output device + routing).
...
| Routing | *(leaf 3)* Core Audio output-device picker + per-channel assignment (LTC vs Track L/R) + the `AVAudioEngine` / `AVAudioSourceNode` playback path, fed by `PlayerEngine.currentTime` | `OnlyCue/LTC/` |
