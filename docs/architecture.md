...
| Routing playback | `LTCAudioOutput` (`@MainActor ObservableObject`) — opens an `AVAudioEngine` on the routed device (`AudioUnitSetProperty(kAudioOutputUnitProperty_CurrentDevice)`, or the system default when `deviceUID == nil`), connects an `AVAudioPlayerNode` with a discrete-layout multichannel float format (`renderFormat(channelCount:sampleRate:)` — `AVAudioFormat`'s simple inits reject 3/5/7… channels), and pumps `LTCSchedule.nextBuffer()` as `makeBuffer(monoSamples:format:channel:)` (mono LTC on the assigned channel, silence elsewhere), keeping `primeCount` (5) buffers scheduled ahead. Two refill paths feed `topUpBuffers()` (schedule until the outstanding lead — tracked by a counter, with a `pumpGeneration` token so a superseded schedule's late completions don't skew it — reaches `primeCount`): each buffer's completion handler (which hops to the main actor — `AVAudioPlayerNode` invokes it off-thread, so a bare `assumeIsolated` traps) **and** a `DispatchSourceTimer` on the main queue firing every `bufferTargetSeconds/2`, so a brief main-actor stall on the handler chain can't drain the queue. `start(at:routing:)` / `stop()` / `update(at:)` (re-cues on the existing engine — no full rebuild on seek); rebuilds on `AVAudioEngineConfigurationChange` (= survives device disconnect). Wired to the transport by `LTCOutputHost`. The engine/device path isn't headless-testable — `LTCSchedule` / `makeBuffer` / `renderFormat` / `buffersToSchedule` / the guards are unit-tested; the live path is verified by running the app | `OnlyCue/LTC/LTCAudioOutput.swift` |
| Transport wiring | `LTCOutputHost` (`.ltcOutput(engine:document:)` on `DocumentView`) — owns an `LTCAudioOutput`; `start`/`stop` on play/pause (with the playhead timecode from `ProjectModel.timecodeSettings` + `LTCRoutingStore.shared`), `update(at:)` on a seek (a `currentTime` jump > 1 s), restart when the routing or timecode settings change mid-playback; no-op when no LTC channel is assigned. Plus the **SMPTE readout** in `TransportBar` (`smpteTimecode` — `timecodeSettings.timecode(atPlaybackSeconds: engine.currentTime).displayString`) | `OnlyCue/UI/LTCOutputHost.swift`, `OnlyCue/UI/TransportBar.swift` |
