...
| Striped-LTC playback | *(leaf 5, partial)* `LTCDecoder` exists; still to wire: an `AVAssetReader` float-sample reader for an imported media's audio track + feeding decoded frames into `PlayerEngine` so striped LTC drives the timeline (generator can override) | `OnlyCue/LTC/`, `OnlyCue/Media/` |
