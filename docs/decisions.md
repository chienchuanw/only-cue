...
**Date**: 2026-07-19
**Status**: Superseded in part (2026-07-19, v0.15.0) — real-rig validation found the target onPC exposes no FTP server, so the FTP transport never shipped. The live **Send to grandMA2…** push streams pure telnet commands instead (`MA2CommandPlanner`: `Store Sequence … Cue`, `Assign … /Trig=Timecode`, `TrigTime` — no XML, no timecode-pool object; the operator's selected TC slot drives the chase). The XML generators and plan builder survive as the input to **Export grandMA2 plugin…** (`MA2PluginGenerator`), which wraps the same plan into a downloadable `.lua` plugin. The "timecode events cannot be created from the command line" finding stands — the command path sidesteps it by trigging cues on the sequence itself rather than populating a timecode show.
...
**Date**: 2026-06-04
**Status**: Accepted
**Decision**: Every media-position readout renders as SMPTE `HH:MM:SS:FF` (`;FF` for drop-frame) at the project framerate: the playhead clock, the cue-list times, the transport current/total/next-cue readouts, the media-sidebar clip durations, the LTC strip, and the Lyric inspector's **Sync Offset** field. The Figma reference used softer per-context formats (compact `m:ss` durations, decimal `HH:MM:SS.mmm` cue/transport times) — we deliberately diverge from the mock here and standardize on framerate-based SMPTE. **Exception:** lyric *line* times stay plain decimal (`M:SS.mmm` queue / `HH:MM:SS.d` placed rows) — they are song-relative authoring content, and "lyrics never carry frames" (ADR-022). The sync **offset** is a media-timecode value (it aligns the song to the clip), so it follows the SMPTE rule, is frame-quantized on edit, and stays **≥ 0** — a song cannot begin before media zero, and per-line display already clamps via `effectiveTime`. The Figma offset frame, which depicts a negative value (`−00:00:00:06`), should be updated to a non-negative one (follow-up).
...
**Amendment (2026-06-15, #551)**: The `Sync Offset` row keeps its inline **Set from Playhead** button, which the original Figma frame (`318:1469`) did not depict. This is a deliberate feature-over-fidelity call (same spirit as ADR-028's SMPTE divergence): setting the offset from the live playhead is the natural authoring gesture and is cheaper than typing SMPTE. Rather than drop the affordance to match the mock, the **Figma component was reconciled to the app** — a Secondary `Button` instance labelled "Set from Playhead" now sits between the label and the offset field in `318:1473`, so design and app agree. The placed-row timestamp column is a fixed width so all lyric text shares one left edge (`318:1490`).
...
**Date**: 2026-05-22
**Status**: Accepted (dual-appearance resolution superseded by ADR-029)
**Amends**: ADR-023
