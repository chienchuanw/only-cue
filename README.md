...
**[v0.6.2](https://github.com/chienchuanw/only-cue/releases/tag/v0.6.2) is the latest release** — small UX fixes with a downloadable DMG: editing a cue's name/number/fade no longer loses the arrow keys to the transport/step shortcuts (the text caret moves normally), and the cue inspector now shows the active clip's filename above the playhead clock.
