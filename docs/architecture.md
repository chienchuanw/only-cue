...
| Routing playback | `LTCAudioOutput` (`@MainActor ObservableObject`) — opens an `AVAudioEngine` on the routed device (`AudioUnitSetProperty(kAudioOutputUnitProperty_CurrentDevice)`, or the system default when `deviceUID == nil`), connects an `AVAudioPlayerNode` with a discrete-layout multichannel float format (`renderFormat(channelCount:sampleRate:)` — `AVAudioFormat`'s simple inits reject 3/5/7… channels), and pumps `LTCSchedule.nextBuffer()` as `makeBuffer(monoSamples:format:channel:)` (mono LTC on the assigned channel, silence elsewhere), keeping `primeCount` (5) buffers scheduled ahead via the completion handler (which hops to the main actor — `AVAudioPlayerNode` invokes it off-thread, so a bare `assumeIsolated` traps). `start(at:routing:)` / `stop()` / `update(at:)` (re-cues on the existing engine — no full rebuild on seek); rebuilds on `AVAudioEngineConfigurationChange` (= survives device disconnect). Wired to the transport by `LTCOutputHost`. The engine/device path isn't headless-testable — `LTCSchedule` / `makeBuffer` / `renderFormat` / the guards are unit-tested; the live path is verified by running the app. Follow-up: a timer-driven refill to `LTCSchedule.targetBufferCount(...)` so a main-actor stall can't underrun | `OnlyCue/LTC/LTCAudioOutput.swift` |
