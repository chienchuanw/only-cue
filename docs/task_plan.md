...
| [#37](https://github.com/chienchuanw/only-cue/issues/37) | Timeline breakdown view (depends on #32) | p1 | 🟡 in progress (4 of 5 leaves in PR [#148](https://github.com/chienchuanw/only-cue/pull/148) — spec/ADR-017 + multi-lane render [`TimelineBreakdownView` + pure `TimelineBreakdownLayout`] + per-Type lane visibility [eye toggle → `CueCommands.setCuePointTypeVisibility`, undoable] + persisted layout [reuses the existing `CuePointType.isVisible` — no schema bump]; `View → Show Timeline Breakdown` `⇧⌘B`. Remaining: leaf 5 — exhaustive layout-fidelity tests at v2/v3 schema + large Type counts) |
