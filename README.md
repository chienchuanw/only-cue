...
| **Epic [#40](https://github.com/chienchuanw/only-cue/issues/40) — custom keyboard shortcuts** | **In progress (1 of 5 leaves).** Data layer landed: a pure `Keymap` (`KeymapAction` × `KeyChord`) persisted as a sparse, forward-tolerant JSON object under `keymap.v1` via `KeymapStore`, with advisory conflict detection (ADR-018). No UI yet — the menu/document shortcuts still use the hardcoded defaults the keymap mirrors. Remaining: Settings → Keyboard editor, per-row conflict UI + reset, editable number-key rows, and rewiring `AppCommands` to read the keymap. |
