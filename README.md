...
**[v0.6.3](https://github.com/chienchuanw/only-cue/releases/tag/v0.6.3) is the latest release** — reliability fixes with a downloadable DMG: closing an edited document now shows the standard "Save / Don't Save / Cancel" prompt instead of dropping changes silently, and "Relink media…" repoints the existing clip (keeping its cues) instead of importing a new empty item.
