...
**[v0.14.2](https://github.com/chienchuanw/only-cue/releases/tag/v0.14.2) is the latest release** — a downloadable DMG fixing the **severe playhead judder (and eventual hang) at high zoom** during follow playback: the waveform, grid, ruler and markers are now rendered once and only translated each frame, so playback stays smooth all the way up to the 64× maximum zoom, with or without LTC.
