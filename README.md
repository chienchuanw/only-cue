...
**[v0.18.0](https://github.com/chienchuanw/only-cue/releases/tag/v0.18.0) is the latest release** — **export your cues as PotPlayer bookmarks**. File ▸ **Export PotPlayer Bookmarks…** copies every video into a folder with a matching `.pbf` beside each, so PotPlayer on Windows auto-loads each cue as a jump-to bookmark titled `[Type] Number Name` — a way to see and scrub a project's songs and cue points on a machine OnlyCue can't run on. The per-cue-type export filter is respected, filename collisions are renamed as matched video/`.pbf` pairs, and each bookmark lands on the exact frame.
